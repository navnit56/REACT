...
function ToDoApp() {
    ...
    const HandleAddTask = () => {
        if (inputValue !== "" && (!task.includes(inputValue)) && editIndex === -1) {
            ...
            const hasDuplicate = editedTask.some((task, i) => 
    task.text === inputValue && i !== editIndex
);

            
          
            if (inputValue !== editedTask) {
                console.log(hasDuplicate)
                if (!hasDuplicate) {
                    
                    setTask(...editedTask, inputValue);
                    setEditIndex(-1);
                } else if (hasDuplicate) {
                    console.log(editedTask + inputValue)
